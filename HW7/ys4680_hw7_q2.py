...
# breath first search
    ...
    def leaves_list(self):
        def yield_leaves_list(root):
            #print( root.data )
            if root.left is None and root.right is None:
                yield root.data
            elif root.left is not None and root.right is None:
                yield from yield_leaves_list( root.left )
            elif root.left is None and root.right is not None:
                yield from yield_leaves_list( root.right )
            else:
                left = yield_leaves_list( root.left )
                right = yield_leaves_list( root.right )
                yield from left
                yield from right
        if self.size == 0:
            return []
        return [node for node in yield_leaves_list(self.root)]
